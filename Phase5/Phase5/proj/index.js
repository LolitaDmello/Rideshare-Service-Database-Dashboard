...
async function checkConnection() {
  try {
    connection = await oracledb.getConnection({
        user: "axa3081",
        password: "Oracle567891",
        connectString: "(DESCRIPTION=(ADDRESS=(COMMUNITY=acaddbprod.uta.edu)(PROTOCOL=TCP)(HOST=acaddbprod.uta.edu)(PORT=1523))(CONNECT_DATA=(SERVICE_NAME=pcse1p.data.uta.edu)(GLOBAL_NAME=pcse1p.data.uta.edu)(SERVER=DEDICATED)))"
    });
...
async function modifyBooking(req, res) {
  try {
      ...
      // var query = 'UPDATE FALL22_S003_12_BOOKING SET FAREAMOUNT = ';
      // var query2 = ' WHERE BID =';
      // var q2 =  (req.body.bid);
      // var query3 = query.concat((req.body.fareamt).concat(query2));
      // var query4 = query3 + '${req.body.bid}';
...
//   try {
//     if (req.body.pcount <= 2) {
//       amt = randomNumber(4,20);//Math.floor(Math.random() * 18) + 5;
//     } else if ((req.body.pcount <= 4)) {
//       amt = randomNumber(20,45);//Math.floor(Math.random() * 25) + 5;
//     }
//     drvid =  String(1000 + randomNumber(1,50));
//     bdate = new Date().toISOString().slice(0, 10);
//     console.log(bdate);
//     console.log(amt);
//     payid = await connection.execute(`SELECT PAYID FROM Fall22_S003_12_PAYMENT WHERE PSGID = '${req.body.psgid}'`);
//       result = await connection.execute(`INSERT INTO FALL22_S003_12_BOOKING VALUES (
//         '${req.body.bid}','${req.body.pickup}','${req.body.pickpin}','${req.body.drop}','${req.body.droppin}','${amt}','${req.body.fare}','${payid}','${req.body.psgid}'
//         ,'','','${req.body.pcount}','')`);
...
//     insertPsg(req, res);
//     res.end('Booking record inserted successfully!');
// });
